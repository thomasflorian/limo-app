...
export class TasksPage {
  ...
  ionViewWillEnter() {
    ...
    this.authListener = this.authService.currentUser.pipe(filter(user => user != null)).subscribe(user => {
      this.driverId = user.id;
      // Gets list of requests of driver.
      this.requests = this.db.collection("drivers").doc(user.id).valueChanges().pipe(filter(resp => resp != null), map<any,any[]>(resp => resp.requests));
      ...
      this.geolocListener = this.geolocation.watchPosition().subscribe((pos) => {
        // Update current position of driver in database.
        this.db.collection("drivers").doc(user.id).update({ position: new firebase.default.firestore.GeoPoint((<Geoposition>pos).coords.latitude, (<Geoposition>pos).coords.longitude) });
  ...
  async taskClick(request) {
    const actionSheet = await this.actionSheetController.create({
      ...
      buttons: [{
        ...
        handler: () => {
          this.requestsService.pickUp({id: request.id, driverId:this.driverId}).subscribe();
        }
      },{
        text: 'No show',
        role: 'destructive',
